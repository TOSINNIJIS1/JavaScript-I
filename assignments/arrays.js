...
console.log(`78, Car 33 is a ${inventory[32].car_year} ${inventory[32].car_make} ${inventory[32].car_model}`);
...
for (let m = 0; m < inventory.length; m++){
	let carModel = inventory[m].car_model;
...
for (y = 0; y < inventory.length; y++){
	const Caryears = inventory[y].car_year;
	carYears.push(Caryears);
	carYears.sort()
